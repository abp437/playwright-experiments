...
test('Employee cannot submit overlapping leave request', async ({ page }) => {
    test.setTimeout(30_000);
    const reason = `AutoLeave-${Date.now()}`;
    const overlapReason = `Overlap-${Date.now()}`;
    const { startLabel, endLabel, startMonthName, endMonthName } = getLeaveDateLabels();
    const applyLeaveDialog = page.locator('[data-playwright-selector="apply-leave-dialog"]');
    ...
    await page.locator('[data-playwright-selector="requests-apply-trigger"]').click();
    await page.locator('[data-playwright-selector="requests-apply-leave"]').click();
    await expect(page.getByRole('heading', { name: 'Apply for Leave' })).toBeVisible({ timeout: 10000 });
    await page
        .locator('[data-playwright-selector="apply-leave-calendar"]')
        .getByRole('grid', { name: startMonthName })
        .getByRole('button', { name: startLabel })
    ...
    await page
        ...
        .getByRole('button', { name: endLabel })
        .click();
    await page.locator('[data-playwright-selector="apply-leave-reason-input"]').fill(overlapReason);
    await page.locator('[data-playwright-selector="apply-leave-submit"]').click();
    await expect(
        page.getByText('Leave request overlaps with an existing leave.'),
    ).toBeVisible();
    await page.keyboard.press('Escape');
    await expect(applyLeaveDialog).toBeHidden();
    await page.reload();
    await page.waitForTimeout(2000);
    const updatedReason = `Adjusted-${Date.now()}`;
    const {
        startLabel: updatedStartLabel,
        endLabel: updatedEndLabel,
        startMonthName: updatedStartMonthName,
        endMonthName: updatedEndMonthName,
    } = getLeaveDateLabels(2);
    ...
    await page
        .locator('[data-playwright-selector="apply-leave-calendar"]')
        .getByRole('grid', { name: updatedStartMonthName })
        .getByRole('button', { name: updatedStartLabel })
        .click();
    await page
        .locator('[data-playwright-selector="apply-leave-calendar"]')
        .getByRole('grid', { name: updatedEndMonthName })
        .getByRole('button', { name: updatedEndLabel })
        .click();
    await page.locator('[data-playwright-selector="apply-leave-reason-input"]').fill(updatedReason);
    await page.locator('[data-playwright-selector="apply-leave-submit"]').click();
