...
test('Manager can reject a leave request', async ({ page }) => {
    ...
    const requestRow = page
        .locator('[data-playwright-selector^="team-request-row-"]')
        .filter({ hasText: reason })
        .first();
    await requestRow.click();

    await page.locator('[data-playwright-selector="review-leave-reject"]').click();
    await expect(page.getByText('Leave rejected successfully')).toBeVisible();
    await logout(page);
});
