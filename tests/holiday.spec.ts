...
async function openAdminSettings(page: Page) {
    ...
    await expect(
        page.getByRole("heading", { name: "Allowed Registration Domains" }),
    ).toBeVisible();
}
...
test.describe("Company Holidays CRUD", () => {
    test("creates, updates, and deletes a company holiday", async ({ page }) => {
        ...
        const holidayHeader = page
            .getByRole("heading", { name: "Company Holidays" })
            .locator("..");
        await holidayHeader
            .locator("button")
        ...
        await expect(page.getByRole("heading", { name: "Update Holiday" })).toBeVisible();
